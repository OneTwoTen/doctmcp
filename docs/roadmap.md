...
**Ưu tiên hiện tại. Tiến độ: 2/8 work item hoàn thành.** Mục tiêu: local runtime hoạt động như một MCP server hoàn chỉnh ở mức tối thiểu và test được offline bằng MCP client thật.
...
**Bước tiếp theo:** #4 — tool `system`. Sau đó tiếp tục `filesystem.read`, `filesystem.write`, `filesystem.delete`, `shell.exec` và acceptance test toàn M1.
