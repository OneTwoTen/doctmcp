...
function waitForMessage(socket: WebSocket, predicate: (message: ReceivedMessage) => boolean = () => true): Promise<ReceivedMessage> {
  return new Promise((resolve, reject) => {
...
function sendHello(socket: WebSocket, sessionId = "test-session", auth?: { deviceId: string; credential: string }): void {
...
function legacyGateway(options: Parameters<typeof createBridgeGateway>[0] = {}): BridgeGateway {
  return createBridgeGateway({ ...options, allowLegacyUnauthenticated: true });
}
...
describe("BridgeGateway", () => {
  ...
  test("treats Bun backpressure status -1 as accepted without retrying", () => {
    let sendCount = 0;
    expect(() => sendWebSocketFrameOnce(() => { sendCount += 1; return -1; })).not.toThrow();
  ...
  test("production default rejects hello without device auth before creating a session", async () => {
    ...
    sendHello(socket);
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.error")).resolves.toMatchObject({
      kind: "bridge.error",
      code: "AUTH_REQUIRED",
    });
    await closePromise;
  ...
  test("authenticated hello binds server-side owner/device identity before ready", async () => {
    ...
    gateway = createBridgeGateway({
      ...
      onSession: (session) => { readySession = session; },
    });
    const socket = await connectClient(gateway);
    sockets.push(socket);
    sendHello(socket, "authenticated-session", { deviceId: DEVICE_ID, credential: "secret-a" });
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.hello.ack")).resolves.toMatchObject({
      kind: "bridge.hello.ack",
      sessionId: "authenticated-session",
    });
    expect(readySession?.state).toBe("ready");
    expect(readySession?.identity).toEqual({ ownerId: "owner-a", deviceId: DEVICE_ID });
  });
  ...
  test("invalid credential fails generically and never exposes ready session", async () => {
    ...
    gateway = createBridgeGateway({
      port: 0,
      authenticateDevice: async () => { throw new Error("secret-a must never be echoed"); },
      onSession: () => { exposed = true; },
    });
    const socket = await connectClient(gateway);
    sockets.push(socket);
    const closePromise = waitForClose(socket);
    sendHello(socket, "bad-auth", { deviceId: DEVICE_ID, credential: "secret-a" });
    const error = await waitForMessage(socket, (message) => message.kind === "bridge.error");
    expect(error).toMatchObject({ kind: "bridge.error", code: "AUTH_FAILED", message: "Device authentication failed" });
  ...
  test("rejects MCP before authentication", async () => {
    ...
    sockets.push(socket);
    const closePromise = waitForClose(socket);
    socket.send(JSON.stringify({ kind: "mcp.message", payload: { jsonrpc: "2.0", id: 1, method: "tools/list" } }));
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.error")).resolves.toMatchObject({
  ...
  test("explicit M2 compatibility accepts legacy handshake", async () => {
    let readySession: BridgeGatewaySession | undefined;
    gateway = legacyGateway({ port: 0, onSession: (session) => { readySession = session; } });
    const socket = await connectClient(gateway);
    sockets.push(socket);
  ...
  test("forwards opaque MCP envelopes in both directions in explicit legacy mode", async () => {
    ...
    sockets.push(socket);
    sendHello(socket);
    await waitForMessage(socket, (message) => message.kind === "bridge.hello.ack");
    ...
    const incoming: BridgeMessage = { kind: "mcp.message", payload: { jsonrpc: "2.0", id: 1, method: "tools/list" } };
    socket.send(JSON.stringify(incoming));
    await expect(incomingPromise).resolves.toEqual(incoming);
    const outgoing: BridgeMessage = { kind: "mcp.message", payload: { jsonrpc: "2.0", id: 1, result: { tools: [] } } };
    const outgoingPromise = waitForMessage(socket, (message) => message.kind === "mcp.message");
  ...
  test("rejects malformed and oversized frames", async () => {
    ...
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.error")).resolves.toMatchObject({ code: "INVALID_MESSAGE" });
    await closePromise;

    socket = await connectClient(gateway);
    sockets.push(socket);
    closePromise = waitForClose(socket);
    socket.send(JSON.stringify({ kind: "bridge.hello", bridgeProtocolVersion: BRIDGE_PROTOCOL_VERSION, role: "local-agent", sessionId: "x".repeat(BRIDGE_MAX_MESSAGE_BYTES) }));
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.error")).resolves.toMatchObject({ code: "MESSAGE_TOO_LARGE" });
    await closePromise;
  });
  ...
  test("rejects unsupported version and handshake timeout", async () => {
    ...
    sockets.push(socket);
    let closePromise = waitForClose(socket);
    socket.send(JSON.stringify({ kind: "bridge.hello", bridgeProtocolVersion: "999", role: "local-agent", sessionId: "bad-version" }));
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.error")).resolves.toMatchObject({ code: "UNSUPPORTED_VERSION" });
    await closePromise;

    socket = await connectClient(gateway);
    sockets.push(socket);
    closePromise = waitForClose(socket);
    await expect(waitForMessage(socket, (message) => message.kind === "bridge.error")).resolves.toMatchObject({ code: "TIMEOUT" });
    await closePromise;
  });
  ...
  test("cleans up remote and repeated local close", async () => {
    ...
    const remoteReason = new Promise<string>((resolve) => { remote.onclose = resolve; });
    socket.close();
