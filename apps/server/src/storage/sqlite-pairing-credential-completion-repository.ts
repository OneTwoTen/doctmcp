...
function snapshot(
  row: CompletionRow,
): PairingCredentialCompletionRecord | null {
  ...
  implements PairingCredentialCompletionRepository
{
  ...
  constructor(
    ...
    if (
      ...
      throw new Error(
        "Pairing completion limits must be positive safe integers.",
      );
    }
  }
  ...
  #nowMs(): number {
    ...
    if (!Number.isFinite(ms))
      throw new Error("Pairing completion clock không hợp lệ.");
    return ms;
  }
  ...
  #count(): number {
    const result = this.#database
      .query("SELECT COUNT(*) AS total FROM pairing_credential_completions")
      .get() as { total: number };
    return result.total;
  }
  ...
  #store(
    ...
    this.#database
      .query(`INSERT INTO pairing_credential_completions
  ...
  async transferReservation(
    ...
    return this.#run(() => {
      ...
      if (source?.state !== "reserved" || this.#row(toPairingSessionId))
        return false;
      return (
        this.#database
          .query(`UPDATE pairing_credential_completions
      ...
      return this.#store(
        {
          ...
          credentialVersion:
            source?.credential_version ?? input.credentialVersion,
  ...
  async advanceRecovery(
    ...
    return this.#run((nowMs) => {
      ...
      if (
        ...
        input.credentialVersion !== old.credential_version + 1
      )
        return null;
      return this.#store(
        {
