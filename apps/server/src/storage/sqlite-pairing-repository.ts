...
import type { SqliteDeviceRepository } from "./sqlite-device-repository";
...
function sessionId(input: string): string {
  ...
  if (!parsed.success)
    return fail("INVALID_PAIRING_RECORD", "pairingSessionId không hợp lệ.");
  return parsed.data;
}
...
function readTime(value: Date, label: string): number {
  ...
  if (!Number.isFinite(ms))
    return fail("INVALID_CLOCK", `${label} không hợp lệ.`);
  return ms;
}
...
function snapshot(row: PairingRow): PairingSession {
  const parsed = pairingSessionSchema.safeParse({
    ...
    ...(row.claimed_at_ms === null
      ? {}
      : { claimedAt: new Date(row.claimed_at_ms) }),
    ...(row.device_id === null ? {} : { deviceId: row.device_id }),
  });
  if (!parsed.success)
    return fail(
      "INVALID_PAIRING_RECORD",
      "Pairing row vi phạm domain invariant.",
    );
 ...
 */
export class SqlitePairingSessionRepository
  implements PairingSessionRepository
{
  ...
  constructor(
    ...
    if (
      ...
      throw new Error(
        "Pairing repository limits must be positive safe integers.",
      );
    }
  }
  ...
  #byId(id: string): PairingRow | null {
    return this.#database
      .query("SELECT * FROM pairing_sessions WHERE pairing_session_id = ?1")
      .get(id) as PairingRow | null;
  }

  #byDigest(codeDigest: string): PairingRow | null {
    return this.#database
      .query("SELECT * FROM pairing_sessions WHERE code_digest = ?1")
  ...
  #prune(atMs: number): number {
    ...
    const deleted = this.#database
      .query(`DELETE FROM pairing_sessions WHERE ${PRUNE_CONDITION}`)
      .run(this.#claimedRetentionMs, atMs);
  ...
  async create(input: CreatePairingRecordInput): Promise<PairingSession> {
    ...
    if (!candidate.success)
      return fail("INVALID_PAIRING_RECORD", "Pairing input không hợp lệ.");
    ...
    return this.#database
      .transaction(() => {
        ...
        const count = this.#database
          .query("SELECT COUNT(*) AS total FROM pairing_sessions")
          .get() as { total: number };
        ...
        this.#database
          .query(`INSERT INTO pairing_sessions
  ...
  async claim(
    input: ClaimPairingRecordInput,
  ): Promise<{ readonly session: PairingSession; readonly device: Device }> {
    const codeDigest = digest(input.codeDigest);
    ...
    const result = this.#database
      .transaction(() => {
        ...
        if (!parsed.success)
          return fail(
            "INVALID_PAIRING_RECORD",
            "Device claim input không hợp lệ.",
          );
        // Intentionally synchronous: await here would commit before the device insert.
        const device = this.#devices.createInTransaction(
          this.#database,
          parsed.data,
        );
        const changed = this.#database
          .query(`UPDATE pairing_sessions
        ...
        if (changed.changes !== 1)
          throw new Error("Pairing claim lost its atomic boundary.");
      ...
      .immediate();
    if (!result)
      return fail("PAIRING_CODE_UNAVAILABLE", "Pairing code không khả dụng.");
    return result;
  }
