...
describePersistentRepositoryContract(
  "SQLite",
  async ({ now, generateDeviceId }) => {
    const db = await openSqliteDatabase({ dataDir: await dataDir() });
    const devices = new SqliteDeviceRepository(db.database, {
      now,
      generateDeviceId,
    });
    return {
      ...
      pairings: new SqlitePairingSessionRepository(db.database, devices, {
        now,
      }),
      close: () => db.close(),
    };
  ...
  test("pending session survives close/reopen and is claimed only once", async () => {
    ...
    try {
      ...
      await new SqlitePairingSessionRepository(first.database, devices).create({
        ...
        expiresAt: new Date(TIME + 60_000),
      });
    } finally {
      first.close();
    }
    ...
    try {
      ...
      const claimed = await pairings.claim({
        ...deviceInput(),
        codeDigest: DIGEST_A,
      });
      ...
      await expect(
        pairings.claim({ ...deviceInput(), codeDigest: DIGEST_A }),
    ...
    try {
      ...
      await expect(
        pairings.create({
          ...
          codeDigest: DIGEST_A,
          createdAt: new Date(TIME + 100_000),
  ...
  test("completion pending, recovery, and ACK survive restart without raw secrets stored", async () => {
    ...
    try {
      ...
      const pairings = new PairingService({
        repository: new SqlitePairingSessionRepository(
          first.database,
          devices,
          {
            now: () => new Date(TIME + 1000),
      ...
      const completion = new PairingCredentialCompletionService({
        ...
        credentialService,
        deviceRepository: devices,
        completionRepository: new SqlitePairingCredentialCompletionRepository(
          first.database,
          {
            now: () => new Date(TIME + 1000),
          },
      ...
      const issued = await completion.claimAndIssue(
        session.pairingCode,
        deviceInput(),
      );
      ...
      expect(
        (
          await new SqlitePairingCredentialCompletionRepository(
            first.database,
            {
              now: () => new Date(TIME + 1000),
            },
      ...
      const dbText = JSON.stringify(
        first.database
          .query("SELECT * FROM pairing_credential_completions")
          .all(),
      );
    ...
    try {
      ...
      await expect(
        credentialService.verify(DEVICE_A, oldSecret),
      ).rejects.toMatchObject({ code: "CREDENTIAL_UNAVAILABLE" });
      expect(
        (await credentialService.verify(DEVICE_A, newSecret)).credential
          .version,
      ).toBe(2);
    ...
    try {
      ...
      await expect(
        completion.resumeClaimedPairing(pairingSessionId, "owner-a"),
      ).rejects.toMatchObject({ code: "PAIRING_COMPLETION_UNAVAILABLE" });
      expect(
        (await credentials.verify(DEVICE_A, newSecret)).identity.ownerId,
      ).toBe("owner-a");
    } finally {
      third.close();
    }
