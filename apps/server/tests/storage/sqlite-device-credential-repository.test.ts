...
describe("M6.4 SQLite credential repository", () => {
    ...
    await withRepositories(async (device, credentials) => {
      ...
      const rotated = await Promise.allSettled([
        ...
        credentials.rotate({
          ...
          expectedVersion: 1,
          credentialId:
            current.credentialId === CREDENTIAL_A ? CREDENTIAL_B : CREDENTIAL_A,
          secretDigest: DIGEST_B,
          rotatedAt: new Date(CREATED_AT.getTime() + 1000),
        }),
      ]);
      expect(
        rotated.filter((result) => result.status === "fulfilled"),
      ).toHaveLength(1);
  ...
  test("global credential IDs giữ unique across devices và restart", async () => {
    ...
    try {
      expect(
        (
          await new SqliteDeviceCredentialRepository(opened.database).getActive(
            DEVICE_A,
          )
        )?.credentialId,
      ).toBe(CREDENTIAL_A);
